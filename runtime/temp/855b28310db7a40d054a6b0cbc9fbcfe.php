...
<div class="search-table layui-form">
    <div class="layui-input-inline tempsTest">
        <input type="text" name="gsname" required  placeholder="请输入规格名称" autocomplete="off" class="layui-input">
    </div>
...
<script>
    layui.use(['table','layer','upload','form'], function(){
        ...
        table.on('tool(testdd)', function(obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'edit') { //修改
                location.href="<?php echo url('Goods/goodsSpecEdit'); ?>?gsid="+data.gsid;
            }else if(layEvent === 'del'){
                layer.confirm("确认删除'"+data.gsname+"'吗？", function(index){
                    $.post(
                        "<?php echo url('Goods/goodsSpecDel'); ?>",
                        data,
                        function(msg){
                            if(msg.code ==1){
                                layer.msg(msg.font);
        ...
        form.on('submit(*)', function(data){
            table.render({
                elem: '#test'
                ,url:"<?php echo url('Goods/goodsSpecList'); ?>"
                ,where:data.field
                ,limit:10
                ,cellMinWidth: 80 //全局定义常规单元格的最小宽度，layui 2.2.1 新增
                ,cols: [[
                    {field:'gsid', width:80, title: 'ID',}
                    ,{field:'gsname', width:200 , title: '规格名称'}
                    ,{field:'spec_content', width:600 , title: '规格内容'}
                    // ,{field:'is_more', width:100, title: '单/多选'}
        ...
        $('.demoTable .layui-btn').on('click', function(){
            location.href="<?php echo url('Goods/goodsSpecAdd'); ?>";
        });
    });
</script>
        </div>
