...
<div class="layui-btn-group demoTable" style="margin-top: 10px">
    <button class="layui-btn layui-btn-normal" data-type="goodsAdd">添加菜品</button>
</div>
...
<script>
    layui.use(['table','layer','upload','form'], function(){
        ...
        table.on('tool(testdd)', function(obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'edit') { //修改
                location.href="<?php echo url('Goods/goodsEdit'); ?>?gid="+data.gid;
                ...
                layer.confirm("确认上架'"+data.name+"'吗？", function(index){
                    $.post(
                        "<?php echo url('Goods/goodsUp'); ?>",
                        data,
                        function(msg){
                            if(msg.code ==1){
                                layer.msg(msg.font);
                ...
                layer.confirm("确认下架'"+data.name+"'吗？", function(index){
                    $.post(
                        "<?php echo url('Goods/goodsDown'); ?>",
                        data,
                        function(msg){
                            if(msg.code ==1){
                                layer.msg(msg.font);
        ...
        form.on('submit(*)', function(data){
            table.render({
                ...
                ,cols: [[
                    ...
                    ,{field:'img', width:150 , title: '菜品图片',templet:'<div><img style="height: 80px;height: 80px" src="__PUBLIC__/{{d.img}}"></div>'}
