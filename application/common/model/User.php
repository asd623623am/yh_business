...
class User extends Model{
    ...
    protected $isNoMustField = ['name'=>'会员名称','phone'=>'手机号','birthday'=>'生日', 'card'=>'会员卡', 'email'=>'邮箱',
                                ...
                                'mini_qrcode'=>'小程序二维码','mini_openid'=>'小程序openid','remarks'=>'备注','label'=>'会员标签',
                                'discount'=>'折扣','pay_money'=>'消费金额','countmoney'=>'累计充值金额','is_type'=>'用户类型',
                                'status'=>'状态'];
    /**
     * Notes: 验证必传字段
     * Class: isVerifyMust'
    ...
    public function isVerifSyncColumn($postData){
        foreach ($this->isVerifSyncColumn as $k => $val){
            ...
            if($k == 'third_wx_openid'){
                $postData['third_wx_openid'] = $postData['wx_openid'];
            }
        }
        return $postData;
    }
